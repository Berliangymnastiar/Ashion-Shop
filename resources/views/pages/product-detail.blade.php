...
<section class="section store-breadcrumbs" data-aos="fade-down" data-delay="100">
<div class="container">
    <div class="row">
        <div class="col-12">
            <nav>
...
<div class="row">
    <div class="col-lg-8" data-aos="zoom-in">
        <transition name="slide-fade" mode="out-in">
            <img :src="photos[activePhoto].url" :key="photos[activePhoto].id" style="" class="w-100 main-image"
                alt="">
        </transition>
    </div>
            ...
            <div class="col-3 col-lg-12 mt-2 mt-lg-0" v-for="(photo, index) in photos" :key="photo.id"
                ...
                <a href="#" @click="changeActive(index)">
                    <img :src="photo.url" class="w-100 thumbnail-image"
                        :class="{ active: index == activePhoto }" alt="" style="max-height: 200px">
                </a>
            </div>
        </div>
    </div>
...
<section class="store-heading">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <h1>{{ $product->name }}</h1>
                <div class="owner">By {{ $product->author }}</div>
            ...
            @auth
               <form action="{{ route('detail-add', $product->id) }}" method="POST" enctype="multipart/form-data">
                   @csrf
                    <button type="submit" class="btn btn-danger px-4 text-white btn-block mb-3">Add to Cart
                    </button>
               </form>
            @else
                <a href="{{ route('login') }}" class="btn btn-danger px-4 text-white btn-block mb-3">Sign in to Add
                </a>
            @endauth
